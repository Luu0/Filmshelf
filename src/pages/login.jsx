...
export default function Login() {
  const { handleLogin, loading, error: backendError } = useLogin()
    ...
    <div className="min-h-screen w-full bg-[#0a0614] flex items-center justify-center px-4 relative">
      ...
      <header className="absolute top-0 left-0 w-full flex items-center px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-white text-2xl font-bold tracking-wide">FILMSHELF</h1>
        </div>
      </header>
      ...
      <div className="relative bg-black/5 backdrop-blur-xl rounded-3xl px-[44px] py-[62px] max-w-lg w-full shadow-[0px_0px_50px_5px_rgba(128,0,255,0.1)] z-10">
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-7">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-gray-300 text-[0.95rem]">Email or Username</label>
            <input
              type="text"
              ...
              className={`
                ...
                ${fieldErrors.emailOrUsername ? "border-red-500 focus:ring-red-500" : "border-white/10 focus:ring-purple-600"}
              `}
            />
            {fieldErrors.emailOrUsername && (
              <span className="text-red-400 text-sm">{fieldErrors.emailOrUsername}</span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className={`
                bg-black/30 border rounded-xl px-5 py-4 
                text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2
                ${fieldErrors.password ? "border-red-500 focus:ring-red-500" : "border-white/10 focus:ring-purple-600"}
              `}
            />
            {fieldErrors.password && (
              <span className="text-red-400 text-sm">{fieldErrors.password}</span>
            )}
